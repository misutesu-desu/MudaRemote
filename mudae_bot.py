        ...
        if not client.claim_right_available and key_mode: # Key mode and no claim right logic
            ...
            for msg in mudae_messages:
                if msg.embeds:
                    embed = msg.embeds[0]
                    if embed.color.value in [16751916, 1360437]:
                        character_name = embed.author.name if embed.author else ""
                        # Prioritize wishlist character
                        if any(wish and wish.lower() in character_name.lower() for wish in client.wishlist):
                            wishlist_claim_message = msg
                        description = embed.description or ""
                        match = re.search(r"\*\*([\d,]+)\*\*<:kakera:", description)
                        if match:
                            kakera_string = match.group(1)
            ...
            # If a wishlist character is found, claim it regardless of kakera value
            if wishlist_claim_message:
                await claim_character(client, channel, wishlist_claim_message)
            elif highest_claim_character_message:
